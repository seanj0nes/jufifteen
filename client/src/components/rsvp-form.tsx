...
export function RsvpForm() {
  ...
  const handleSubmit = async (data: InsertRsvp) => {
    ...
    try {
      // Enviamos los datos a Google Sheets usando OAuth
      ...
      // Manejamos de manera especial los errores de autorización
      if (error.message && (error.message.includes('token') || error.message.includes('autenticación') || error.message.includes('autorización'))) {
        setIsAuthenticated(false);
        toast({
          title: "Error de autenticación",
          description: "Por favor, inicia sesión nuevamente con Google",
          variant: "destructive",
        });
      } else {
        toast({
      ...
      {!isAuthenticated ? (
        <div className="bg-black/50 backdrop-blur-sm p-6 rounded-lg mb-6">
          <h3 className="text-lg text-center text-white mb-4">
            Para confirmar tu asistencia, iniciá sesión con Google
          </h3>
